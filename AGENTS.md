...
- The Fabric runtime pair (`cordis-fabric`, `cordis-fabric-api`) is required from the profile as peer dependencies; fixed GitHub Release tarballs appear only in `devDependencies` for local checks. Do not bundle a second Fabric copy. This repository's `pnpm-workspace.yaml` enables `strictPeerDependencies` for local installs and checks; the consuming profile must install the Fabric bundle separately, because this workspace setting is not transferred to the profile. The profile's Fabric bundle supplies the `fabric-dsh` launcher and bootstrap row.
