...
Copy source-controlled template files while excluding `.git/`, `node_modules/`, `lib/`, temporary files, and package-manager stores. Do not use an unguarded recursive delete. Preserve this build path:

- development/CI and release: `tsdown.config.ts` compiles the host and browser entries directly from `src/`, while `tsc --noEmit` performs typechecking only.
...
- `tsconfig.json`, `tsconfig.host.json`, `tsconfig.client.json`, and `scripts/*.mjs`: local compiler and boundary-verification topology;
