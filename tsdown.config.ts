...
/**
 * ex-setting is a dual-face package: the host crawler half (index, invariant)
 * plus the browser client bundle. The browser half must ship in the dsh
 * closure-factory artifact (window.__ModuleLoader__.load({id, factory})) —
 * the same format the official client bundles emit — because the web shell
 * loads `/plugins/<id>/client.js` as a classic script and resolves every
 * cross-package value import through the loader module table. Plain ESM
 * bundles cannot load there.
 *
...
const CLIENT_ID = '@deepseek-ai/dsh-ex-setting'
