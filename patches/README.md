...
Keep the patch version exact, document why the patch is required, and remove it when the upstream dependency contains the fix. A patch that affects the release build must be present in source control and covered by clean-install, `pnpm run build`, and pack verification. Do not add an empty `patchedDependencies` block when the plugin has no patches.
